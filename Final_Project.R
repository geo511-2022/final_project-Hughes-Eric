...
median_income_shape$INTPTLAT = as.numeric(median_income_shape$INTPTLAT)
...
nc <- median_income_shape %>% 
  st_transform(4326) %>%
  st_intersection(Buffalo_border)
...
#Median Income Plot
median_income_plot <- ggplot(median_income_shape) + 
...
ggmap(basemap) + 
  geom_sf(data = nc, aes( fill = Median.Household.Income), 
             size = .025, alpha = 0.7, inherit.aes = F) +
